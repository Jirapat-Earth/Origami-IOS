...
class TimeSample extends StatefulWidget {
  const TimeSample({super.key, required this.employee});
...
class _TimeSampleState extends State<TimeSample> {
        ...
        children: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        SizedBox(width: 4),
                        Text(
                          "${timeStamp?.comp_name ?? ''} (${timeStamp?.branch_name ?? ''})",
                          style: GoogleFonts.openSans(
                            fontSize: 14,
                            color: Colors.white,
                            fontWeight: FontWeight.w500,
          ...
          Expanded(
            ...
            child: GoogleMap(
              initialCameraPosition: CameraPosition(
                target: LatLng(
                    latDouble, lngDouble), // กำหนดการซูมที่ตำแหน่งที่ตั้งไว้
                zoom: 18,
              ),
          ...
          Expanded(
              ...
              child: Column(
                ...
                children: [
                  if (timeStamp?.stamp_in == '-' && timeStamp?.stamp_in == '-')
                    GestureDetector(
                      onTap: () => (_isOut == true)?null:_pickImage(ImageSource.camera),
                      child: const CircleAvatar(
                          radius: 50,
                          backgroundImage: AssetImage(
                              'assets/images/stamp/stamp_button_in.png')),
                    ),
                  if (timeStamp?.stamp_in != '-' && timeStamp?.stamp_in == timeStamp?.stamp_out)
                    GestureDetector(
                      onTap: () => (_isOut == true)?null:_pickImage(ImageSource.camera),
                      child: const CircleAvatar(
                          radius: 50,
                  ...
                  if (timeStamp?.stamp_in != timeStamp?.stamp_out)
                    GestureDetector(
                      child: const CircleAvatar(
                          radius: 50,
                          backgroundImage: AssetImage(
  ...
  static double circumDou = 40.0;
  static double latDouble = 13.734185;
  static double lngDouble = 100.626831;
  static GetTimeStampSim? timeStamp;
  Future<GetTimeStampSim> fetchGetTimeStampSim() async {
    final uri =
        Uri.parse("https://www.origami.life/api/get_time_stamp_sim.php");
    ...
    if (response.statusCode == 200) {
      ...
      _addInTime = timeStamp?.stamp_in??'';
      if (timeStamp?.stamp_in == '-' && timeStamp?.stamp_in == '-'){
        _addOutTime = '-';
      }else if (timeStamp?.stamp_in != '-' && timeStamp?.stamp_in == timeStamp?.stamp_out){
        _addOutTime = '-';
      }else{
        _addOutTime = timeStamp?.stamp_out??'';
      }
      circumDou = double.parse(timeStamp?.comp_circumference ?? '');
      latDouble = double.parse(timeStamp?.lat ?? '');
      lngDouble = double.parse(timeStamp?.lng ?? '');
      // });

      // แปลง JSON ตอบสนองเป็นอ็อบเจกต์ CurriculumData โดยตรง
      return GetTimeStampSim.fromJson(jsonResponse);
    } else {
